...
from proteinworkshop.models.graph_encoders.layers.util import FusionLayer
...
class GeoMHCIIModel(nn.Module):
    ...
    :param model (str): model name. Available model names are ``ESM-1b``, ``ESM-1v``, ``ESM-1b-regression``, and ``ESM-2`` series.
    :param post_embed_fusion (bool): whether to use cross attention to fuse ESM embeddings with input features
    ...
    def __init__(
        ...
        super(GeoMHCIIModel, self).__init__()
        ...
        self.post_embed_fusion = post_embed_fusion
        self.finetune = finetune

        if self.post_embed_fusion: 
            self.fusion = FusionLayer(emb_dim, self.output_dim, num_heads, dropout)
        else:
            self.fusion = nn.Sequential(
                nn.LazyLinear(emb_dim),
    ...
    def forward(
        ...
        """
        if self.finetune:
            esm_embedding = self.esm_embed(batch, device)
        else:
            with torch.no_grad():
                esm_embedding = self.esm_embed(batch, device)
        ...
        if self.post_embed_fusion:
            h = self.fusion(esm_embedding, h, batch) # (n, d) -> (n, d)
        else:
            h = self.fusion(torch.concatenate([esm_embedding, h], dim=-1))
